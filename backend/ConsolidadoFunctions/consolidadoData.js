...
const fetchConsolidadoData = async (month = "", year = "") => {
    try {
		...
		const tabla3 = await query(`SELECT * FROM 3_sena LIMIT 1`);
        ...
        const indexedTabla3 = indexBy(tabla3, 'ID_PUNTO_ATENCIÓN');
...
const joinTables = (tabla1, tabla2, tabla3, tabla4) => {
	...
	for (const id of Object.keys(tabla2)) {
		const registroTabla2 = tabla2[id];
